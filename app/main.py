"""
API FastAPI : Dailymotion (yt-dlp/ffmpeg) -> Whisper (SRT) -> LlamaIndex (RAG)
...
app = FastAPI(
    title="Audio RAG API",
    description="Ingestion vidéo -> transcription -> RAG avec LlamaIndex",
    version="1.0.0",
)

rag = RAGEngine(persist_dir=str(BASE_DIR / "index"))

# Stockage des jobs en mémoire (à remplacer par Redis/DB en production)
JOBS: dict[str, dict] = {}


def _process_video(job_id: str, url: str, language: str, whisper_model: str):
    """Pipeline complet exécuté en tâche de fond."""
    try:
...
def ingest(req: IngestRequest, background_tasks: BackgroundTasks):
    """Lance le pipeline complet pour une URL Dailymotion (asynchrone)."""
...
def status(job_id: str):
    """Consulte l'état d'un job (queued / downloading / transcribing / indexing / done / error)."""
    job = JOBS.get(job_id)
    if not job:
...
def get_srt(job_id: str):
    """Télécharge le fichier de sous-titres .srt généré."""
    job = JOBS.get(job_id)
...
def ask(req: AskRequest):
    """Interroge le corpus indexé (RAG) sur l'ensemble des vidéos ingérées."""
